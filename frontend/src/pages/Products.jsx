import { useEffect, useMemo, useState } from 'react'
import { api } from '../api'
import { Loading, LoadError, Empty } from '../components/common'
import ProductCard from '../components/ProductCard'

export default function Products() {
  const [q, setQ] = useState('')
  const [brand, setBrand] = useState('')
  const [hasClaims, setHasClaims] = useState('')
  const [state, setState] = useState({ data: null, loading: true, error: null })
  const [brands, setBrands] = useState([])

  // 首屏拉一次全量，提取品牌下拉选项
  useEffect(() => {
    api.products().then((list) => {
      setBrands([...new Set(list.map((p) => p.brand))].sort())
    }).catch(() => {})
  }, [])

  useEffect(() => {
    let alive = true
    setState((s) => ({ ...s, loading: true, error: null }))
    const timer = setTimeout(() => {
      api.products({ q, brand, has_claims: hasClaims })
        .then((data) => alive && setState({ data, loading: false, error: null }))
        .catch((e) => alive && setState({ data: null, loading: false, error: e.message }))
    }, 250)
    return () => { alive = false; clearTimeout(timer) }
  }, [q, brand, hasClaims])

  const claimFilters = useMemo(() => [
    ['', '全部'],
    ['true', '有宣称依据'],
    ['false', '无宣称摘要'],
  ], [])

  return (
    <div>
      <div className="card">
        <div className="flex flex-col md:flex-row gap-3">
          <input
            className="input md:flex-1"
            placeholder="搜索产品名 / 品牌…"
            value={q}
            onChange={(e) => setQ(e.target.value)}
          />
          <select className="input md:w-56" value={brand} onChange={(e) => setBrand(e.target.value)}>
            <option value="">全部品牌</option>
            {brands.map((b) => <option key={b} value={b}>{b}</option>)}
          </select>
          <div className="flex gap-1 bg-bg rounded-[10px] p-1 self-start">
            {claimFilters.map(([v, label]) => (
              <button
                key={v}
                onClick={() => setHasClaims(v)}
                className={`btn-page whitespace-nowrap ${hasClaims === v ? 'btn-page-active' : ''}`}
              >
                {label}
              </button>
            ))}
          </div>
        </div>
      </div>

      {state.loading && <div className="card"><Loading /></div>}
      {state.error && <div className="card"><LoadError error={state.error} /></div>}
      {state.data && state.data.length === 0 && <Empty text="没有匹配的产品，换个关键词或放宽过滤条件试试" />}
      {state.data && state.data.length > 0 && (
        <>
          <div className="text-xs text-ink-3 mb-3">共 {state.data.length} 款产品</div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {state.data.map((p) => <ProductCard key={p.id} p={p} />)}
          </div>
        </>
      )}
    </div>
  )
}
