...
match 串对产品名做子串模糊匹配；同名多条时按「有序产品（有位次关联）优先 +
brand 匹配」消歧（如理肤泉 B5：有序「B5多效修复霜（Cicaplast Baume B5）」
胜出，无序「理肤泉新B5多效修复霜」不误写）。无法唯一消歧时不猜写，计入
unmatched。写入 product.price_current / product.spec，并落一条 PricePoint
（date=今天、source=source_url、is_manual=True）；同日同源重复执行更新价格
而非重复插点（幂等）。
...
def load_prices(session: Session, path: Path = PRICE_PATH) -> dict:
    ...
    for item in data["items"]:
        product = _find_product(session, item["match"], item.get("brand"))
        if product is None:
